...
export class ApplicationsComponent implements OnInit {
  ...
  readonly applications = signal<DirectoryApplication[]>([]); readonly loading = signal(true); readonly page = signal(1); readonly pageSize = 10;
  ...
  load(): void { this.loading.set(true); this.directory.getAllApplications().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: (applications) => { this.applications.set(applications); this.loading.set(false); this.resetPage(); }, error: () => this.loading.set(false) }); }
