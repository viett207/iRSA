...
export class CandidatesComponent implements OnInit {
  ...
  readonly candidates = signal<DirectoryCandidate[]>([]); readonly loading = signal(true); readonly page = signal(1); readonly pageSize = 10; searchTerm = '';
  ...
  load(): void { this.loading.set(true); this.directory.getAllCandidates().pipe(takeUntilDestroyed(this.destroyRef)).subscribe({ next: (candidates) => { this.candidates.set(candidates); this.loading.set(false); this.resetPage(); }, error: () => this.loading.set(false) }); }
